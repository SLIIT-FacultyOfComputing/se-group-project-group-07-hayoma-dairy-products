...
export default function AdminOrdersDashboard() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setIsLoading(true)
        // BACKEND INTEGRATION: Fetch orders from API
        // const response = await fetch('/api/orders')
        // const data = await response.json()
        // setOrders(data)

        // Extract unique shops from orders
        // const uniqueShops = Array.from(new Set(data.map((order: Order) => order.shopName)))
        // setShops(uniqueShops)

        // For now, set empty array until backend is connected
        setOrders([])
        setShops([])
      } catch (error) {
  ...
  const handleUpdateStatus = async (id: string, newStatus: Order["status"]) => {
    try {
      // BACKEND INTEGRATION: Update order status API call
